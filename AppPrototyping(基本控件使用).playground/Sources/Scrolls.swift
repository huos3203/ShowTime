...
public class ScrollView: NSObject, UIScrollViewDelegate {
	...
	public init(device: Device, imageName: String?, scrollDirection: ScrollDirection, width: CGFloat?, height: CGFloat?) {
		...
		if let widthAnchor = width {
			...
			view.widthAnchor.constraint(equalToConstant:device.frame().width).isActive = true
        ...
        view.contentSize = CGSize(width:contentWidth!, height:contentHeight!) // TODO: if more views are added, contentSize needs to be updated to show all scrollable area and proper scroller bars. TB.
	...
	func setupScrollers() {
        ...
        verticalScroller = UIView(frame: CGRect(x:verticalX, y:verticalY, width:scrollerThickness, height:verticalHeight))
		verticalScroller?.backgroundColor = .black
		verticalScroller?.alpha = 0.5
		verticalScroller?.layer.cornerRadius = 3
		
        horizontalScroller = UIView(frame: CGRect(x:horizontalX, y:horizontalY, width:horizontalHeight, height:scrollerThickness))
		horizontalScroller?.backgroundColor = .black
