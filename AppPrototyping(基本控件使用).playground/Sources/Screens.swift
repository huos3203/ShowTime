...
//	var activityIndicators: [ActivityIndicator]? = nill // TODO: add ActivityIndicator Class.
	...
	public init(device: Device, type: ScreenType, appearance: Appearance) {
        ...
        mainSV = makeVerticalSV(view:view)
	...
	public func splash(_ name: String) {
		if let splashImage = UIImage(named: name) {
	...
	mutating func list() {
		...
		setConstraints(visualLanguage:horizontalVL, views: viewDict)
		var verticalVL = keys.map{"V:[\($0)(44)]"}
		verticalVL[0] = "V:[statusBar(20)]" // StatusBar height to 20 instead of 44.
        verticalVL.remove(at:4) // Remove tableView constraint. TODO remove this and add vertical height for each toolbar based on an enum. TB. case: status = 20, case: tab = 49, case tool = 44. TB.
		print("verticalVL: \(verticalVL)")
		setConstraints(visualLanguage:verticalVL, views: viewDict)
	}
}
